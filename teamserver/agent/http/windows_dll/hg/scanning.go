...
func ScanPortsCommon(host string) []int {
	...
	for _, port := range topPorts {
		go func() {
			_, err := net.DialTimeout("tcp", host+":"+strconv.Itoa(port), time.Duration(500)*time.Millisecond)
			if err == nil {
